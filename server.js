...
app.get('/api/notes', (req, res) => {
    res.sendFile(path.join(__dirname, '/db/db.json')) // read the db.json file and return all saved notes as json
});
...
app.post('/api/notes', (req, res) => {
    ...
    // parameters: filename, encoding, callback function. source: https://nodejs.org/dist/latest-v6.x/docs/api/fs.html#fs_fs_readfile_file_options_callback
        if (err) {
            ...
            // write the new set of notes back to the JSON file and send the response of parsedNotes as JSON so that it gets written back to the page when the save button is clicked
...
app.delete('/api/notes/:id', (req, res) => {
    fs.readFile('./db/db.json', 'utf8', (err, data) => {
        if (err) {
            ...
            // rewrite the ids of all notes. if any note except the last one is deleted, the ids are no longer going to range from 1 to length, and they need to be kept that way for the post route to work properly.
